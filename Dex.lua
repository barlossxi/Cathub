...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if presentClasses["BasePart"] or presentClasses["Model"] then
			context:AddRegistered("Check_Isnetworkowner")
	...
	Explorer.InitRightClick = function()
		...
		context:Register("Check_Isnetworkowner", {
			Name = "Check IsNetworkOwner",
			IconMap = Explorer.MiscIcons,
			Icon = "Reference",
			OnClick = function()
				local sList = selection.List
				for _, node in next, sList do
					...
					if part then
						if isnetworkowner(part) then
							print(part:GetFullName() .. " is owned by you. Testing physics (Velocity = 0, 50, 0)")
							pcall(function()
							...
							print(part:GetFullName() .. " is NOT owned by you.")
						end
					end
				end
			end,
